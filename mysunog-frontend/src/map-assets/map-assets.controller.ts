...
export class MapAssetsController {
  ...
  // ==================== HYDRANTS ====================
  @Get('hydrants')
  getHydrants() { return this.mapAssetsService.findAllHydrants(); }

  @UseGuards(AuthGuard('jwt'))
  @Post('hydrants')
  @UseInterceptors(FileInterceptor('photo', { storage: photoStorage, fileFilter: imageFileFilter }))
  ...
  @Patch('hydrants/:id')
  @UseInterceptors(FileInterceptor('photo', { storage: photoStorage, fileFilter: imageFileFilter }))
  updateHydrant(@Param('id') id: string, @Body() body: any, @UploadedFile() file?: Express.Multer.File) {
    if (file) body.photoUrl = `/uploads/map-assets/${file.filename}`;
    const data: any = { ...body };
  ...
  // ==================== HOTSPOTS ====================
  @Get('hotspots')
  getHotspots() { return this.mapAssetsService.findAllHotspots(); }
  ...
  @Patch('hotspots/:id')
  @UseInterceptors(FileInterceptor('photo', { storage: photoStorage, fileFilter: imageFileFilter }))
  updateHotspot(@Param('id') id: string, @Body() body: any, @UploadedFile() file?: Express.Multer.File) {
    if (file) body.photoUrl = `/uploads/map-assets/${file.filename}`;
    const data: any = { ...body };
    if (body.lat !== undefined) data.lat = Number(body.lat);
    if (body.lng !== undefined) data.lng = Number(body.lng);
  ...
  // ==================== BFP STATIONS ====================
  @Get('stations')
  getStations() { return this.mapAssetsService.findAllStations(); }
  ...
  @Patch('stations/:id')
  @UseInterceptors(FileInterceptor('photo', { storage: photoStorage, fileFilter: imageFileFilter }))
  updateStation(@Param('id') id: string, @Body() body: any, @UploadedFile() file?: Express.Multer.File) {
    if (file) body.photoUrl = `/uploads/map-assets/${file.filename}`;
    const data: any = { ...body };
    if (body.lat !== undefined) data.lat = Number(body.lat);
    if (body.lng !== undefined) data.lng = Number(body.lng);
