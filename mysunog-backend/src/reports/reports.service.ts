...
export class ReportsService {
  ...
  async generateSingleIncidentPdf(id: number): Promise<any> {
    ...
    return new Promise((resolve) => {
      ...
      doc
        .font('Helvetica-Bold')
        .text('Incident ID:', leftColX, startY)
        .font('Helvetica')
        .text(`#${incident.id}`, leftColX + 100, startY);
      doc
        .font('Helvetica-Bold')
        .text('Date:', rightColX, startY)
        .font('Helvetica')
        .text(incident.incidentDate, rightColX + 80, startY);
      startY += 20;

      doc
        .font('Helvetica-Bold')
        .text('Time:', leftColX, startY)
        .font('Helvetica')
        .text(incident.incidentTime, leftColX + 100, startY);
      doc
        .font('Helvetica-Bold')
        .text('Barangay:', rightColX, startY)
        .font('Helvetica')
        .text(incident.barangay, rightColX + 80, startY);
      startY += 20;

      doc
        .font('Helvetica-Bold')
        .text('Coordinates:', leftColX, startY)
        .font('Helvetica')
        .text(
      ...
      doc
        .fontSize(14)
        .font('Helvetica-Bold')
        .text('Assessment & Damages', leftColX, startY);
      startY += 20;
      doc.fontSize(12);

      doc
        .font('Helvetica-Bold')
        .text('Cause:', leftColX, startY)
        .font('Helvetica')
        .text(incident.cause || 'Unknown', leftColX + 100, startY);
      doc
        .font('Helvetica-Bold')
        .text('Property Type:', rightColX, startY)
        .font('Helvetica')
        .text(incident.propertyType || 'N/A', rightColX + 100, startY);
      startY += 20;

      doc
        .font('Helvetica-Bold')
        .text('Est. Damage:', leftColX, startY)
        .font('Helvetica')
        .text(
          `P ${Number(incident.estimatedDamage || 0).toLocaleString()}`,
          leftColX + 100,
          startY,
        );
      doc
        .font('Helvetica-Bold')
        .text('Casualties:', rightColX, startY)
        .font('Helvetica')
        .text(incident.casualties?.toString() || '0', rightColX + 100, startY);
      startY += 20;

      doc
        .font('Helvetica-Bold')
        .text('Injuries:', leftColX, startY)
        .font('Helvetica')
        .text(incident.injuries?.toString() || '0', leftColX + 100, startY);
      startY += 30;
      ...
      doc
        .fontSize(12)
        .font('Helvetica')
        .text(incident.remarks || 'No remarks provided.', leftColX, startY, {
          align: 'justify',
          width: 500,
        });
